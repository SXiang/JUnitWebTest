...
public class SurveyorDataReader extends BaseDataReader {
	...
	public Integer getSurveyorDataRowID(String surveyorUnitDesc) throws Exception {
		Integer rowID = -1;
		Integer rowCount = getRowCount();
		for (int i = 1; i <= rowCount; i++) {
			SurveyorDataRow dataRow = getDataRow(i);
			if (dataRow.description.equalsIgnoreCase(surveyorUnitDesc)) {
				rowID = i;
				break;
			}
		}
	...
	public SurveyorDataRow getDataRow(Integer dataRowID) throws Exception {
		...
		description = ActionArguments.evaluateArgForFunction(description);
