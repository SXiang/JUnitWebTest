...
public class ManageSurveyorHistoriesPageTest extends SurveyorBaseTest {
	...
	/**
	 * Test Case ID: TC134_TC1254_AddNoteMaxCharLimit
	 * More than 1500 characters not allowed in Note
	 * field present on Manage Add Surveyor History screen
	 * 
	 */
	@Test
	...
	public void TC134_TC1254_AddNoteMaxCharLimit(String user, String pwd) {
		...
		if (user.equalsIgnoreCase("administrator")) {
			tcID = "TC134";
			strNote = tcID + " Automation Note "
					+ testSetup.getFixedSizeRandomNumber(9);
		...
		Log.info("\nRunning - "
				+ tcID
				+ "_AddNoteMaxCharLimit - Test Description: More than 1500 characters not allowed in Note field present on Manage Add Surveyor History screen\n");
		...
		assertTrue("Administrator not able to add new history note!",
				manageSurveyorHistoriesPage.findExistingHistoryNote(SQACUS,
						SQACUSLOC, SQACUSLOCSUR, strNote1500chars));
		
		manageSurveyorHistoriesPage.addNewHistoryNote(surveyorUnit, strNote1501chars);
		assertFalse("Administrator was able to add new history note!",
				manageSurveyorHistoriesPage.findExistingHistoryNote(SQACUS,
						SQACUSLOC, SQACUSLOCSUR, strNote1501chars));
	}
}