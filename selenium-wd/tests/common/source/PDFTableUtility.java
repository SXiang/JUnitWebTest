...
public class PDFTableUtility extends PDFUtility{
	...
	public static enum PDFTable {		
		...
		CRSPECTABLE ("Map Height & Width:.*",0,"LISA Investigation Complete",false,6),
	...
	public boolean isSortedASC(String[] strArray){
		return isSortedASC(strArray,0);
	}
	
	public boolean isSortedASC(String[] strArray,int beginAt){
		return isSortedASC(strArray,beginAt,strArray.length-1);
	}
	
	public boolean isSortedASC(String[] strArray, int beginAt, int endAt){
		boolean sorted = true;
	...
	public static void main(String[] args) {
	    ...
	    expectedPDFTableMap.put(fileName, PDFTable.CRSPECTABLE);
		...
		try {
			...
			for (String filePath : pdfFilesInDirectory) {
				...
				// Capture table data from PDF
				if (expectedTableMap.containsKey(filename)) {
					pdfTableUtility.testExtractPDFTable_validPDF(Paths.get(filePath).toString(), 
	...
	private void testExtractPDFTable_validPDF(String filePath, String[][] expectedTable, PDFTable pTable) throws IOException {
	  ...
	  if(pTable.equals(PDFTable.LISAINDICATIONTABLE)||pTable.equals(PDFTable.LISAINVESTIGATIONTABLE)){
		  Assert.assertTrue(isSortedASC(colValue1, 1));
		  Assert.assertTrue(isSortedASC(colValue2, 1));
	  }
	 ...
	 // Retrieve a  cell in the first field
